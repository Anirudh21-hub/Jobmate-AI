...
def send_cold_email(email_list, contact_info):
    ...
    email_writer_memory = OpenAIMemory(file_path="data/text.txt")
    ...
    email_writer_task = Task(
        ...
        instructions="Use the resume information and job description to write a cold email of 250 words to be sent to the recruiter. [IMPORTANT!] send the response in html use bullets for points and beautify it professionally. Return only the email. Don't leave any field empty. My personal details are : "
        + contact_info,
        log_output=True,
    ...
    LinearSyncPipeline(
        ...
        tasks=[email_writer_task, send_email_task],
    ).run()
